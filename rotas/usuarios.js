...
const verificaUsuario = (req, res, next) => {
    try {
      ...
      const tokenUsuario = req.tokenData.usuario.id;
  
      if (idRota !== tokenUsuario) {
        return res.status(401).send('Ação não permitida para este usuário');
      }
...
const rotas_usuarios = () => {
    ...
    router.get('/usuarios/:id', ValidaToken, verificaUsuario);
    router.put('/usuarios/:id', ValidaToken, verificaUsuario);
    router.patch('/usuarios/:id', ValidaToken, verificaUsuario);
    router.delete('/usuarios/:id', ValidaToken, verificaUsuario);
